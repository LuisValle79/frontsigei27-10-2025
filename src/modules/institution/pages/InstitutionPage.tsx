  ...
  const handleDelete = async (institutionId: string) => {
    if (window.confirm('¿Estás seguro de que quieres eliminar esta institución?')) {
      try {
        ...
        setError(err instanceof Error ? err.message : 'Error al eliminar institución');
      }
    }
  };

  const handleRestore = async (institutionId: string) => {
    if (window.confirm('¿Estás seguro de que quieres restaurar esta institución?')) {
      try {
        await institutionService.restoreInstitution(institutionId);
        loadInstitutions();
      } catch (err) {
        setError(err instanceof Error ? err.message : 'Error al restaurar institución');
      }
    }
  };
                          ...
                          <div className="flex items-center justify-end gap-2">
                            ...
                            {!isDeleted && (
                              <button
                                ...
                                className="bg-yellow-600 hover:bg-yellow-700 text-white px-3 py-1 rounded text-xs font-medium transition duration-200"
                              >
                                Editar
                              </button>
                            )}
                            {isDeleted ? (
                              <button
                                onClick={() => handleRestore(institution.institutionId)}
                              ...
                              <button
                                onClick={() => handleDelete(institution.institutionId)}
